...
def solution(nums):
    ...
    for i in xrange(0, len(nums)):
        for j in xrange(i+1, len(nums)):
            m1 = magnitude(reduce([copy.deepcopy(nums[i]), copy.deepcopy(nums[j])]))
            largest = max(largest, m1)

            m2 = magnitude(reduce([copy.deepcopy(nums[j]), copy.deepcopy(nums[i])]))
            largest = max(largest, m2)
